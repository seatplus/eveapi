...
class EsiClientSetup
{
    ...
    public function get(?EsiAuthentication $authentication = null): EsiClient
    {
        ...
        if ($authentication) {
            $authentication->client_id = config('eveapi.config.esi.eve_client_id');
            $authentication->secret = config('eveapi.config.esi.eve_client_secret');
        }

        return new EsiClient($authentication);
    }
}
